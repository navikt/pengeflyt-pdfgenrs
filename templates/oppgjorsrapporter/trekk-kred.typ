...
#set page(
  ...
  footer: context [
    #set text(size: 8pt)
    Trekkoppgjørsrapport (T14)
    #h(1fr)
    #counter(page).display((side, total) => [side #side av #total], both: true)
  ]
)

#let sum-row(..celler) = celler.pos().map(c => table.cell(
  fill: rgb("f2f3f5"),
    ...
    ..{
      ...
      let tabellrad(arkivref, t, rowspan) = {
        (
          ...
          t.fnr,
          t.navn,
          if "periodeFra" in t and "periodeTil" in t [#t.periodeFra - #t.periodeTil],
          t.belop,
          table.cell(rowspan: rowspan, breakable: false)[],
        )
      }

    for (arkivref, trekk, refnrsum) in init {
//        (
      let (..initTrekk, sisteTrekk) = trekk
      for t in initTrekk { tabellrad(arkivref, t, 1) }
      tabellrad(arkivref, sisteTrekk, 2)
      sum-row([], [], [], [Delsum arkivref #arkivref], [], refnrsum)
    }

    let (..initSisteArkivrefTrekk, sisteArkivrefSisteTrekk) = sisteArkivref.trekk
    for t in initSisteArkivrefTrekk { tabellrad(sisteArkivref.arkivref, t, 1) }
    tabellrad(sisteArkivref.arkivref, sisteArkivrefSisteTrekk, 3)
    sum-row([], [], [], [Delsum arkivref #sisteArkivref.arkivref], [], sisteArkivref.refnrsum)
